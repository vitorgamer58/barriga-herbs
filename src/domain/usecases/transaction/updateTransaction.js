...
const updateTransaction = (injection) => usecase('Update Transaction', {
  ...
  'Check if it is a valid Transaction before update': step((ctx) => {
    ...
    return newTransaction.isValid() ? Ok() : Err.invalidEntity({
...
module.exports = herbarium.usecases
  .add(updateTransaction, 'UpdateTransaction')
  .metadata({
    group: 'Transaction', operation: herbarium.crud.update, entity: Transaction, REST: { put: '/v1/transactions/:id' }
  })
  .usecase;
